...
## 🎯 Acceptance Criteria
- [ ] AC1: Service correctly calls the `/deputados` endpoint of the Chamber API.
- [ ] AC2: Data for all 513 active deputies is fetched.
- [ ] AC3: Data is stored in the `deputies` table (id, name, party, state, photo_url).
- [ ] AC4: Duplicate handling implemented (upsert logic).
...
## 📝 Tasks / Subtasks
- [ ] Task 1: Create PostgreSQL table `deputies` in Supabase as defined in `docs/architecture/database-schema.md`.
- [ ] Task 2: Create a sync function (or script) in `src/functions/sync-api/` to fetch data from the Chamber API.
- [ ] Task 3: Map API response to the `deputies` table schema.
- [ ] Task 4: Implement the sync logic using Supabase client to UPSERT data.
